...
st.set_page_config(page_title="Trading Plan Pro - Fixed Dividend", page_icon="📈", layout="wide")
...
st.title("🦅 DASHBOARD TRADING PLAN HARIAN & JADWAL DIVIDEN")
st.write(f"Analisis Teknikal & Info Dividen Akurat — Update: {date.today().strftime('%d %B %Y')}")
...
# --- FUNGSI BARU: AMBIL DATA DIVIDEN HISTORIS & TERBARU ---
def ambil_dividen_dari_finance(ticker_obj):
    try:
        # Menarik data kalender dividen resmi dari Yahoo Finance
        hist_div = ticker_obj.dividends
        if not hist_div.empty:
            # Ambil data dividen paling terakhir/terbaru
            terakhir_ex_date = hist_div.index[-1]
            nominal = hist_div.iloc[-1]
            
            # Hitung estimasi Cum-Date (1 hari kerja sebelum Ex-Date)
...
try:
    ...
    with st.spinner(f'Sinkronisasi data saham & dividen {ticker_clean}...'):
        # Inisialisasi Objek Ticker
        ticker_obj = yf.Ticker(ticker_code)
        
        # Download Data Harga & Dividen
    ...
    if df_raw is not None and not df_raw.empty:
        ...
        vol_avg = float(pd.Series(volume_arr).rolling(window=20).mean().iloc[-1])
        ...
        with col_right:
            ...
            # --- SEKSI INFO DIVIDEN YANG SUDAH DIPERBAIKI ---
            st.write("---")
