...
# Konfigurasi Layout
st.set_page_config(page_title="Trading Plan Pro V5", layout="wide", page_icon="🦅")

# --- FUNGSI PENGAMBILAN DATA (MULTI-TIMEFRAME) ---
@st.cache_data(ttl=300)
def get_market_data(ticker):
    try:
        # Ambil data Intraday (5M) dan Daily (1D)
        ...
        # Bersihkan format Multi-Index yfinance versi terbaru
...
def calculate_advanced_indicators(df):
    """Kalkulasi Indikator Kelas Institusi"""
    # 1. EMA & Bollinger Bands
    df['EMA20'] = df['Close'].ewm(span=20, adjust=False).mean()
    df['BB_Mid'] = df['Close'].rolling(20).mean()
    df['BB_Std'] = df['Close'].rolling(20).std()
    df['Upper'] = df['BB_Mid'] + (df['BB_Std'] * 2)
    df['Lower'] = df['BB_Mid'] - (df['BB_Std'] * 2)
    
    # 2. VWAP (Volume Weighted Average Price) Harian
    ...
    # 3. ATR (Volatilitas untuk Stop Loss)
...
if not df_5m.empty and not df_1d.empty:
    ...
    # Analisis Multi-Timeframe (Cek Tren Utama di Daily)
    ...
    # --- TABS LAYOUT ---
    tab1, tab2 = st.tabs(["📊 Analisis Teknikal & Chart", "📋 Action Plan Detail"])
    
    with tab1:
        # Plotly Chart Intraday
        fig = go.Figure()
        
        # Candlestick
        fig.add_trace(go.Candlestick(
            x=df_5m.index, open=df_5m['Open'], high=df_5m['High'], low=df_5m['Low'], close=df_5m['Close'], name="Harga 5M"
        ))
        # VWAP Line (Sangat Penting)
        fig.add_trace(go.Scatter(
            x=df_5m.index, y=df_5m['VWAP'], line=dict(color='#3b82f6', width=2), name='VWAP (Bandar Line)'
        ))
        # EMA Line
        fig.add_trace(go.Scatter(
            x=df_5m.index, y=df_5m['EMA20'], line=dict(color='#fbbf24', width=1.5, dash='dot'), name='EMA 20'
        ))
    ...
    with tab2:
        ...
        with col_rules:
            ...
            if tren_harian == "DOWNTREND 🔴":
                st.error("⚠️ **Peringatan Keras:** Tren harian sedang turun. Jangan melakukan day trade, risiko nyangkut sangat tinggi.")
            elif entry < curr_5m['VWAP']:
                st.warning("⏳ **Wait & See:** Harga masih di bawah VWAP. Institusi belum menunjukkan tanda akumulasi. Tunggu hingga harga breakout ke atas garis biru (VWAP).")
            elif entry > curr_5m['VWAP'] and entry > curr_5m['EMA20']:
                st.success("🔥 **Clear to Launch:** Tren harian Bullish, harga intraday di atas EMA20 dan di atas VWAP. Ini adalah setup probabilitas tinggi.")
            else:
                st.info("🔄 **Fase Konsolidasi:** Market sedang tidak punya arah yang jelas. Lebih baik pantau saham lain.")
            
            st.caption("\n\n*Jangan lupa validasi kembali sentimen market, antrean bid/offer, dan broker summary melalui platform trading utama Anda (seperti Stockbit) sebelum mengeksekusi order.*")

else:
    st.error("Gagal menarik data. Pastikan format kode saham benar (contoh: BBCA).")
